...
data + tests + strategies + backtest ──→ run/run_walk_forward.py (端到端)
```
...
## run/run_walk_forward.py — 端到端 Walk-Forward 回测
...
- `run/run_walk_forward.py` 的 Walk-Forward 耗时较长（滚动重估），38 只 ETF 全扫描约需数分钟
